...
class Descriptor
{
    ...
    public static function buildFromProto($proto, $file_proto, $containing, $custom_json_names = [])
    {
        ...
        foreach ($proto->getNestedType() as $nested_proto) {
            $desc->addNestedType(\Google\Protobuf\Internal\Descriptor::buildFromProto($nested_proto, $file_proto, $message_name_without_package, $custom_json_names));
        }
        // Handle nested enum.
        foreach ($proto->getEnumType() as $enum_proto) {
            $desc->addEnumType(\Google\Protobuf\Internal\EnumDescriptor::buildFromProto($enum_proto, $file_proto, $message_name_without_package, $custom_json_names));
        }
        // Handle oneof fields.
        $index = 0;
