...
class Message
{
        ...
        foreach ($array as $key => $value) {
            ...
            if ($field->isMap()) {
                ...
                foreach ($value as $tmp_key => $tmp_value) {
                    ...
                    if ($value_field->getType() == GPBType::ENUM && is_string($tmp_value) && is_null($value_field->getEnumType()->getValueByJsonName($tmp_value)) && $ignore_unknown) {
                        continue;
                    }
                ...
                foreach ($value as $tmp) {
                    ...
                    // ignored if ignore_unknown is set.
                    if ($field->getType() == GPBType::ENUM && is_string($tmp) && is_null($field->getEnumType()->getValueByJsonName($tmp)) && $ignore_unknown) {
                        continue;
                    }
    ...
    public function parseFromJsonStream($input, $ignore_unknown)
    {
        ...
        try {
            ...
            throw new \Google\Protobuf\Internal\GPBDecodeException($e->getMessage(), $e->getCode(), $e);
        }
    }
    /**
     * @ignore
    ...
    public function serializeToJsonStream(&$output)
    {
        ...
        if (is_a($this, 'Google\Protobuf\Any')) {
            ...
            $first = \true;
            foreach ($fields as $field) {
                if ($this->existField($field, $options) || \Google\Protobuf\Internal\GPBUtil::hasJsonValue($this)) {
                    if ($first) {
                        $first = \false;
                    } else {
                        $output->writeRaw(",", 1);
        ...
        switch ($field->getType()) {
            ...
            case GPBType::ENUM:
                ...
                if ($options & PrintOptions::ALWAYS_PRINT_ENUMS_AS_INTS) {
                    ...
                    if (!is_null($enum_value_desc)) {
                        $name = \Google\Protobuf\Internal\GPBJsonWire::formatEnumValueName($enum_value_desc);
                        $encoded = json_encode($name, \JSON_UNESCAPED_UNICODE);
                        $size += strlen($encoded);
                    } else {
    ...
    private function fieldJsonByteSize($field, $options = 0)
    {
        ...
        } elseif ($this->existField($field, $options) || \Google\Protobuf\Internal\GPBUtil::hasJsonValue($this)) {
            if (!\Google\Protobuf\Internal\GPBUtil::hasSpecialJsonMapping($this)) {
                $size += 3;
                // size for "\"\":".
                if ($options & PrintOptions::PRESERVE_PROTO_FIELD_NAMES) {
