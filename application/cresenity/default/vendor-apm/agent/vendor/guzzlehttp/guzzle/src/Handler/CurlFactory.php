...
namespace CresenityDevCloudAPMVendor\GuzzleHttp\Handler;

use CresenityDevCloudAPMVendor\GuzzleHttp\Exception\ConnectException;
use CresenityDevCloudAPMVendor\GuzzleHttp\Exception\RequestException;
use CresenityDevCloudAPMVendor\GuzzleHttp\Multiplexing;
use CresenityDevCloudAPMVendor\GuzzleHttp\Promise as P;
use CresenityDevCloudAPMVendor\GuzzleHttp\Promise\FulfilledPromise;
use CresenityDevCloudAPMVendor\GuzzleHttp\Promise\PromiseInterface;
use CresenityDevCloudAPMVendor\GuzzleHttp\Psr7;
use CresenityDevCloudAPMVendor\GuzzleHttp\Psr7\LazyOpenStream;
use CresenityDevCloudAPMVendor\GuzzleHttp\Psr7\Uri;
use CresenityDevCloudAPMVendor\GuzzleHttp\TransferStats;
use CresenityDevCloudAPMVendor\GuzzleHttp\TransportSharing;
use CresenityDevCloudAPMVendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \CresenityDevCloudAPMVendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \CresenityDevCloudAPMVendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \CresenityDevCloudAPMVendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options): void
    {
        if (\array_key_exists('stream_context', $options)) {
            \CresenityDevCloudAPMVendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
