...
class AlwaysRecordSampler implements SamplerInterface
{
    ...
    {
        $rootSamplerSamplingResult = $this->root->shouldSample($parentContext, $traceId, $spanName, $spanKind, $attributes, $links);
        if ($rootSamplerSamplingResult->getDecision() === SamplingResult::DROP) {
            return new SamplingResult(SamplingResult::RECORD_ONLY, $rootSamplerSamplingResult->getAttributes(), $rootSamplerSamplingResult->getTraceState());
        }
