...
final class MetricAggregator implements MetricAggregatorInterface
{
    ...
    public function __construct(private readonly ?AttributeProcessorInterface $attributeProcessor, private readonly AggregationInterface $aggregation, private readonly ?ExemplarReservoirInterface $exemplarReservoir = null)
    {
    }
    #[\Override]
    public function record(float|int $value, AttributesInterface $attributes, ContextInterface $context, int $timestamp): void
    {
        $filteredAttributes = $this->attributeProcessor !== null ? $this->attributeProcessor->process($attributes, $context) : $attributes;
        $raw = $filteredAttributes->toArray();
        $index = $raw !== [] ? serialize($raw) : 0;
        $this->attributes[$index] ??= $filteredAttributes;
        $this->aggregation->record($this->summaries[$index] ??= $this->aggregation->initialize(), $value, $attributes, $context, $timestamp);
    ...
    {
        $exemplars = $this->exemplarReservoir ? $this->exemplarReservoir->collect($this->attributes) : [];
