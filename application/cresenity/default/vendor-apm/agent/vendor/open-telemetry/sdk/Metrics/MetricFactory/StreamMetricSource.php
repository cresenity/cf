...
{
    public function __construct(private readonly StreamMetricSourceProvider $provider, private readonly int $reader)
    {
    }
    #[\Override]
    ...
    {
        return new Metric($this->provider->instrumentationLibrary, $this->provider->resource, $this->provider->view->name, $this->provider->view->unit, $this->provider->view->description, $this->provider->stream->collect($this->reader));
    }
