...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Attribute\LogRecordAttributeValidator;
...
class ReadableLogRecord extends LogRecord
{
    ...
    public function __construct(private readonly InstrumentationScopeInterface $scope, private readonly LoggerSharedState $loggerSharedState, LogRecord $logRecord)
    {
        parent::__construct($logRecord->body);
        $this->timestamp = $logRecord->timestamp;
        $this->observedTimestamp = $logRecord->observedTimestamp ?? (int) (microtime(\true) * (float) LogRecord::NANOS_PER_SECOND);
        ...
        $this->convertedAttributes = $this->loggerSharedState->getLogRecordLimits()->getAttributeFactory()->builder($logRecord->attributes, new LogRecordAttributeValidator())->build();
    }
