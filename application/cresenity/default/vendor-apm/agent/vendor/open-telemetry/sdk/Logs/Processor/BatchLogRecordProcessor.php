...
class BatchLogRecordProcessor implements LogRecordProcessorInterface
{
    ...
    public function __construct(private readonly LogRecordExporterInterface $exporter, private readonly ClockInterface $clock, int $maxQueueSize = self::DEFAULT_MAX_QUEUE_SIZE, int $scheduledDelayMillis = self::DEFAULT_SCHEDULE_DELAY, int $exportTimeoutMillis = self::DEFAULT_EXPORT_TIMEOUT, int $maxExportBatchSize = self::DEFAULT_MAX_EXPORT_BATCH_SIZE, private readonly bool $autoFlush = \true, ?MeterProviderInterface $meterProvider = null)
    {
        ...
        $this->processorAttributes = [OtelIncubatingAttributes::OTEL_COMPONENT_TYPE => OtelIncubatingAttributes::OTEL_COMPONENT_TYPE_VALUE_BATCHING_LOG_PROCESSOR, OtelIncubatingAttributes::OTEL_COMPONENT_NAME => OtelIncubatingAttributes::OTEL_COMPONENT_TYPE_VALUE_BATCHING_LOG_PROCESSOR . '/' . spl_object_id($this)];
        if ($meterProvider === null) {
            ...
            $meter->createObservableUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_PROCESSOR_LOG_QUEUE_CAPACITY, '{log_record}', 'The maximum number of log records the queue of a given log record processor can hold')->observe(function (ObserverInterface $observer): void {
                $observer->observe($this->maxQueueSize, $this->processorAttributes);
            });
            $meter->createObservableUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_PROCESSOR_LOG_QUEUE_SIZE, '{log_record}', 'The number of log records in the queue of a given log record processor')->observe(function (ObserverInterface $observer): void {
                $observer->observe($this->queueSize, $this->processorAttributes);
            });
            $this->logProcessedCounter = $meter->createCounter(OtelIncubatingMetrics::OTEL_SDK_PROCESSOR_LOG_PROCESSED, '{log_record}', 'The number of log records for which the processing has finished, either successful or failed');
            $this->logInflightCounter = $meter->createUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_EXPORTER_LOG_INFLIGHT, '{log_record}', 'The number of log records which were passed to the exporter, but that have not been exported yet');
            $this->logExportedCounter = $meter->createCounter(OtelIncubatingMetrics::OTEL_SDK_EXPORTER_LOG_EXPORTED, '{log_record}', 'The number of log records for which the export has finished, either successful or failed');
        }
    }
    #[\Override]
