...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Future\ErrorFuture;
...
use CresenityDevCloudAPMVendor\Psr\Http\Client\NetworkExceptionInterface;
...
final class PsrTransport implements TransportInterface
{
     ...
     */
    public function __construct(private readonly ClientInterface $client, private readonly RequestFactoryInterface $requestFactory, private readonly StreamFactoryInterface $streamFactory, private readonly string $endpoint, private readonly string $contentType, private readonly array $headers, private readonly array $compression, private readonly int $retryDelay, private readonly int $maxRetries)
    {
    }
    #[\Override]
    ...
    public function send(string $payload, ?CancellationInterface $cancellation = null): FutureInterface
    {
        ...
        $request = $this->requestFactory->createRequest('POST', $this->endpoint)->withBody($this->streamFactory->createStream($body))->withHeader('Content-Type', $this->contentType);
        if ($appliedEncodings) {
        ...
        for ($retries = 0;; $retries++) {
            ...
            try {
                ...
                if ($response->getStatusCode() >= 400 && $response->getStatusCode() < 500 && !in_array($response->getStatusCode(), [408, 429], \true)) {
            ...
            $sec = (int) $delay;
            $nsec = (int) (($delay - (float) $sec) * 1000000000.0);
            /** @psalm-suppress ArgumentTypeCoercion */
            if (time_nanosleep($sec, $nsec) !== \true) {
                return new ErrorFuture(new RuntimeException('Export cancelled', 0, $e));
            }
        }
        assert(isset($response));
        try {
            $body = PsrUtils::decode($response->getBody()->__toString(), self::parseContentEncoding($response));
        } catch (Throwable $e) {
            return new ErrorFuture($e);
        }
