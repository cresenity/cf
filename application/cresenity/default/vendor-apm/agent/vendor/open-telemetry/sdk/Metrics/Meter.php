...
use function CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Util\closure;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\Exemplar\ExemplarFilterInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricRegistration\MultiRegistryRegistration;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricRegistration\RegistryRegistration;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricRegistry\MetricRegistryInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricRegistry\MetricWriterInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\StalenessHandler\MultiReferenceCounter;
...
final class Meter implements MeterInterface, Configurable
{
     ...
     */
    public function __construct(private readonly MetricFactoryInterface $metricFactory, private readonly ResourceInfo $resource, private readonly ClockInterface $clock, private readonly StalenessHandlerFactoryInterface $stalenessHandlerFactory, private readonly iterable $metricRegistries, private readonly ViewRegistryInterface $viewRegistry, private readonly ?ExemplarFilterInterface $exemplarFilter, private readonly MeterInstruments $instruments, private readonly InstrumentationScopeInterface $instrumentationScope, private readonly MetricRegistryInterface $registry, private readonly MetricWriterInterface $writer, private readonly ArrayAccess $destructors, ?Configurator $configurator = null)
    {
    ...
    {
        [$instrument, $referenceCounter] = $this->createSynchronousWriter(InstrumentType::COUNTER, $name, $unit, $description, $advisory);
    ...
    public function createObservableCounter(string $name, ?string $unit = null, ?string $description = null, $advisory = [], callable ...$callbacks): ObservableCounterInterface
    {
        ...
        [$instrument, $referenceCounter] = $this->createAsynchronousObserver(InstrumentType::ASYNCHRONOUS_COUNTER, $name, $unit, $description, $advisory);
    ...
    {
        [$instrument, $referenceCounter] = $this->createSynchronousWriter(InstrumentType::HISTOGRAM, $name, $unit, $description, $advisory);
    ...
    {
        [$instrument, $referenceCounter] = $this->createSynchronousWriter(InstrumentType::GAUGE, $name, $unit, $description, $advisory);
    ...
    public function createObservableGauge(string $name, ?string $unit = null, ?string $description = null, $advisory = [], callable ...$callbacks): ObservableGaugeInterface
    {
        ...
        [$instrument, $referenceCounter] = $this->createAsynchronousObserver(InstrumentType::ASYNCHRONOUS_GAUGE, $name, $unit, $description, $advisory);
        foreach ($callbacks as $callback) {
            $this->writer->registerCallback(closure($callback), $instrument);
            $referenceCounter->acquire(\true);
        }
    ...
    {
        [$instrument, $referenceCounter] = $this->createSynchronousWriter(InstrumentType::UP_DOWN_COUNTER, $name, $unit, $description, $advisory);
    ...
    public function createObservableUpDownCounter(string $name, ?string $unit = null, ?string $description = null, $advisory = [], callable ...$callbacks): ObservableUpDownCounterInterface
    {
        ...
        [$instrument, $referenceCounter] = $this->createAsynchronousObserver(InstrumentType::ASYNCHRONOUS_UP_DOWN_COUNTER, $name, $unit, $description, $advisory);
        foreach ($callbacks as $callback) {
            $this->writer->registerCallback(closure($callback), $instrument);
            $referenceCounter->acquire(\true);
        }
    ...
    private function createSynchronousWriter(string|InstrumentType $instrumentType, string $name, ?string $unit, ?string $description, array $advisory = []): array
    {
        ...
        return $instruments->writers[$instrumentationScopeId][$instrumentId] = [$instrument, $stalenessHandler, new RegisteredInstrument(!$this->config->isEnabled(), $this)];
    }
    /**
     * @return array{Instrument, ReferenceCounterInterface, RegisteredInstrument}
     */
    private function createAsynchronousObserver(string|InstrumentType $instrumentType, string $name, ?string $unit, ?string $description, array $advisory): array
    {
        ...
        return $instruments->observers[$instrumentationScopeId][$instrumentId] = [$instrument, $stalenessHandler, new RegisteredInstrument(!$this->config->isEnabled(), $this)];
    }
    ...
    {
        $views = $this->viewRegistry->find($instrument, $this->instrumentationScope) ?? [new ViewProjection($instrument->name, $instrument->unit, $instrument->description, null, null)];
