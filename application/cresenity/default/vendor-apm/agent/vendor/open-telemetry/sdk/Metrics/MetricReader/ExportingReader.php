...
namespace CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricReader;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\AggregationInterface;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\DefaultAggregationProviderInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\DefaultAggregationProviderTrait;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricExporterInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricFactory\StreamMetricSourceProvider;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricMetadataInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricReaderInterface;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricSourceRegistryUnregisterInterface;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SemConv\Incubating\Metrics\OtelIncubatingMetrics;
...
final class ExportingReader implements MetricReaderInterface, MetricSourceRegistryInterface, MetricSourceRegistryUnregisterInterface, DefaultAggregationProviderInterface
{
    ...
    public function __construct(private readonly MetricExporterInterface $exporter, ?MeterProviderInterface $meterProvider = null)
    {
        $this->readerAttributes = [OtelIncubatingAttributes::OTEL_COMPONENT_TYPE => OtelIncubatingAttributes::OTEL_COMPONENT_TYPE_VALUE_PERIODIC_METRIC_READER, OtelIncubatingAttributes::OTEL_COMPONENT_NAME => OtelIncubatingAttributes::OTEL_COMPONENT_TYPE_VALUE_PERIODIC_METRIC_READER . '/' . spl_object_id($this)];
        if ($meterProvider === null) {
            ...
            $this->exporterAttributes = [OtelIncubatingAttributes::OTEL_COMPONENT_NAME => (new \ReflectionClass($this->exporter))->getShortName()];
            $meter = $meterProvider->getMeter('io.opentelemetry.sdk', schemaUrl: Version::VERSION_1_36_0->url());
            $this->collectionDuration = $meter->createHistogram(OtelIncubatingMetrics::OTEL_SDK_METRIC_READER_COLLECTION_DURATION, 's', 'The duration of the collect operation of the metric reader');
            $this->dataPointInflightCounter = $meter->createUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_EXPORTER_METRIC_DATA_POINT_INFLIGHT, '{data_point}', 'The number of metric data points which were passed to the exporter, but that have not been exported yet');
            $this->dataPointExportedCounter = $meter->createCounter(OtelIncubatingMetrics::OTEL_SDK_EXPORTER_METRIC_DATA_POINT_EXPORTED, '{data_point}', 'The number of metric data points for which the export has finished, either successful or failed');
        }
    }
    ...
    private function doCollect(): bool
    {
        ...
        $durationSeconds = (hrtime(\true) - $startNs) / 1000000000;
