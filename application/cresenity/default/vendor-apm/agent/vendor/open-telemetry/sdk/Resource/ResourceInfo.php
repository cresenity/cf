...
class ResourceInfo
{
    ...
    {
        $copyOfAttributesAsArray = array_slice($this->attributes->toArray(), 0);
        ...
        return serialize(['schemaUrl' => $this->schemaUrl, 'attributes' => $copyOfAttributesAsArray]);
    }
    /**
    ...
    private static function mergeSchemaUrl(?string $old, ?string $updating): ?string
    {
        ...
        self::logWarning('Merging resources with different schema URLs', ['old' => $old, 'updating' => $updating]);
        return null;
    }
    /**
     * @codeCoverageIgnore
    ...
    {
        BcUtil::triggerMethodDeprecationNotice(__METHOD__, 'defaultResource', ResourceInfoFactory::class);
