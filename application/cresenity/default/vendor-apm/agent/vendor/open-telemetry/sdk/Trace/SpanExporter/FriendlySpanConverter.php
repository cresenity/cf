...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\EventInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\LinkInterface;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\StatusDataInterface;
use ReflectionClass;
class FriendlySpanConverter implements SpanConverterInterface
{
    ...
    {
        return [self::CODE_ATTR => $status->getCode(), self::DESCRIPTION_ATTR => $status->getDescription()];
    }
    /**
    ...
    private function convertEvents(array $events): array
    {
        ...
        foreach ($events as $event) {
            $result[] = [self::NAME_ATTR => $event->getName(), self::TIMESTAMP_ATTR => $event->getEpochNanos(), self::ATTRIBUTES_ATTR => $this->convertAttributes($event->getAttributes())];
        }
        return $result;
    }
    /**
    ...
    private function convertLinks(array $links): array
    {
        ...
        foreach ($links as $link) {
            $result[] = [self::CONTEXT_ATTR => $this->convertContext($link->getSpanContext()), self::ATTRIBUTES_ATTR => $this->convertAttributes($link->getAttributes())];
        }
        return $result;
    }
    /**
