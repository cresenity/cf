...
use CresenityDevCloudAPMVendor\OpenTelemetry\Context\ContextStorageInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Attribute\AttributesFactoryInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Instrumentation\InstrumentationScopeFactoryInterface;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricFactory\StreamFactory;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Metrics\MetricRegistry\MetricRegistry;
...
final class MeterProvider implements MeterProviderInterface
{
     ...
     */
    public function __construct(?ContextStorageInterface $contextStorage, private readonly ResourceInfo $resource, private readonly ClockInterface $clock, AttributesFactoryInterface $attributesFactory, private readonly InstrumentationScopeFactoryInterface $instrumentationScopeFactory, private readonly iterable $metricReaders, private readonly ViewRegistryInterface $viewRegistry, private readonly ?ExemplarFilterInterface $exemplarFilter, private readonly StalenessHandlerFactoryInterface $stalenessHandlerFactory, private readonly MetricFactoryInterface $metricFactory = new StreamFactory(), private ?Configurator $configurator = null)
    {
    ...
    public function getMeter(string $name, ?string $version = null, ?string $schemaUrl = null, iterable $attributes = []): MeterInterface
    {
        ...
        $meter = new Meter($this->metricFactory, $this->resource, $this->clock, $this->stalenessHandlerFactory, $this->metricReaders, $this->viewRegistry, $this->exemplarFilter, $this->instruments, $this->instrumentationScopeFactory->create($name, $version, $schemaUrl, $attributes), $this->registry, $this->writer, $this->destructors, $this->configurator);
