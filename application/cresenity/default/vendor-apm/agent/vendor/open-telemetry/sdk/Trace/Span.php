...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Attribute\AttributesBuilderInterface;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Dev\Compatibility\Util as BcUtil;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Exception\StackTraceFormatter;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\SpanSuppression\NoopSuppressionStrategy\NoopSuppression;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\SpanSuppression\SpanSuppression;
use CresenityDevCloudAPMVendor\OpenTelemetry\SemConv\Incubating\Attributes\OtelIncubatingAttributes;
use Throwable;
final class Span extends API\Span implements ReadWriteSpanInterface
{
     ...
     */
    private function __construct(private string $name, private readonly API\SpanContextInterface $context, private readonly InstrumentationScopeInterface $instrumentationScope, private readonly int $kind, private readonly API\SpanContextInterface $parentSpanContext, private readonly SpanLimits $spanLimits, private readonly SpanProcessorInterface $spanProcessor, private readonly ResourceInfo $resource, private AttributesBuilderInterface $attributesBuilder, private array $links, private int $totalRecordedLinks, private readonly int $startEpochNanos, private readonly SpanSuppression $spanSuppression, ?UpDownCounterInterface $spanLiveCounter = null)
    {
     ...
     */
    public static function startSpan(string $name, API\SpanContextInterface $context, InstrumentationScopeInterface $instrumentationScope, int $kind, API\SpanInterface $parentSpan, ContextInterface $parentContext, SpanLimits $spanLimits, SpanProcessorInterface $spanProcessor, ResourceInfo $resource, AttributesBuilderInterface $attributesBuilder, array $links, int $totalRecordedLinks, int $startEpochNanos, SpanSuppression $spanSuppression = new NoopSuppression(), ?UpDownCounterInterface $spanLiveCounter = null): self
    {
        $span = new self($name, $context, $instrumentationScope, $kind, $parentSpan->getContext(), $spanLimits, $spanProcessor, $resource, $attributesBuilder, $links, $totalRecordedLinks, $startEpochNanos !== 0 ? $startEpochNanos : Clock::getDefault()->now(), $spanSuppression, $spanLiveCounter);
    ...
    public function addLink(SpanContextInterface $context, iterable $attributes = []): self
    {
        ...
        $this->links[] = new Link($context, $this->spanLimits->getLinkAttributesFactory()->builder($attributes)->build());
        return $this;
    }
    /** @inheritDoc */
    #[\Override]
    ...
    public function recordException(Throwable $exception, iterable $attributes = [], ?int $timestamp = null): self
    {
        ...
        $timestamp ??= Clock::getDefault()->now();
        $eventAttributesBuilder = $this->spanLimits->getEventAttributesFactory()->builder(['exception.type' => $exception::class, 'exception.message' => $exception->getMessage(), 'exception.stacktrace' => StackTraceFormatter::format($exception)]);
        foreach ($attributes as $key => $value) {
    ...
    public function end(?int $endEpochNanos = null): void
    {
        ...
        $span = clone $this;
        $this->hasEnded = \true;
        // prevent further modifications to the span by async code
    ...
    {
        return new ImmutableSpan($this, $this->name, $this->links, $this->events, $this->attributesBuilder->build(), $this->totalRecordedLinks, $this->totalRecordedEvents, $this->status, $this->endEpochNanos ?? 0, $this->hasEnded);
    }
    /** @inheritDoc */
    #[\Override]
    ...
    private function checkForDroppedElements(): void
    {
        ...
        if ($spanData->getTotalDroppedLinks() || $spanData->getTotalDroppedEvents() || $spanData->getAttributes()->getDroppedAttributesCount() || $droppedEventAttributes || $droppedLinkAttributes) {
            self::logWarning('Dropped span attributes, links or events', ['trace_id' => $spanData->getTraceId(), 'span_id' => $spanData->getSpanId(), 'attributes' => $spanData->getAttributes()->getDroppedAttributesCount(), 'links' => $spanData->getTotalDroppedLinks(), 'link_attributes' => $droppedLinkAttributes, 'events' => $spanData->getTotalDroppedEvents(), 'event_attributes' => $droppedEventAttributes]);
        }
    }
}
