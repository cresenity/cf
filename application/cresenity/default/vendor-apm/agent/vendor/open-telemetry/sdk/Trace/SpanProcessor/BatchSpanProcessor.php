...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\SpanDataInterface;
...
class BatchSpanProcessor implements SpanProcessorInterface
{
    ...
    public function __construct(private readonly SpanExporterInterface $exporter, private readonly ClockInterface $clock, int $maxQueueSize = self::DEFAULT_MAX_QUEUE_SIZE, int $scheduledDelayMillis = self::DEFAULT_SCHEDULE_DELAY, int $exportTimeoutMillis = self::DEFAULT_EXPORT_TIMEOUT, int $maxExportBatchSize = self::DEFAULT_MAX_EXPORT_BATCH_SIZE, private readonly bool $autoFlush = \true, ?MeterProviderInterface $meterProvider = null)
    {
        ...
        $this->processorAttributes = [OtelIncubatingAttributes::OTEL_COMPONENT_TYPE => OtelIncubatingAttributes::OTEL_COMPONENT_TYPE_VALUE_BATCHING_SPAN_PROCESSOR, OtelIncubatingAttributes::OTEL_COMPONENT_NAME => OtelIncubatingAttributes::OTEL_COMPONENT_TYPE_VALUE_BATCHING_SPAN_PROCESSOR . '/' . spl_object_id($this)];
        if ($meterProvider === null) {
            ...
            $meter->createObservableUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_PROCESSOR_SPAN_QUEUE_CAPACITY, '{span}', 'The maximum number of spans the queue of a given span processor can hold')->observe(function (ObserverInterface $observer): void {
                $observer->observe($this->maxQueueSize, $this->processorAttributes);
            });
            $meter->createObservableUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_PROCESSOR_SPAN_QUEUE_SIZE, '{span}', 'The number of spans in the queue of a given span processor')->observe(function (ObserverInterface $observer): void {
                $observer->observe($this->queueSize, $this->processorAttributes);
            });
            $this->spanProcessedCounter = $meter->createCounter(OtelIncubatingMetrics::OTEL_SDK_PROCESSOR_SPAN_PROCESSED, '{span}', 'The number of spans for which the processing has finished, either successful or failed');
            $this->spanInflightCounter = $meter->createUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_EXPORTER_SPAN_INFLIGHT, '{span}', 'The number of spans which were passed to the exporter, but that have not been exported yet');
            $this->spanExportedCounter = $meter->createCounter(OtelIncubatingMetrics::OTEL_SDK_EXPORTER_SPAN_EXPORTED, '{span}', 'The number of spans for which the export has finished, either successful or failed');
        }
    }
    #[\Override]
    ...
    {
        return !$this->flush->isEmpty() || $this->autoFlush && !$this->queue->isEmpty() || $this->autoFlush && $this->nextScheduledRun !== null && $this->clock->now() > $this->nextScheduledRun;
    }
