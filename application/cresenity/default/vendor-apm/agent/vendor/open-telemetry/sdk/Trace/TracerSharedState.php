...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\SpanProcessor\MultiSpanProcessor;
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\SpanProcessor\NoopSpanProcessor;
...
final class TracerSharedState
{
    ...
    public function __construct(private readonly IdGeneratorInterface $idGenerator, private readonly ResourceInfo $resource, private readonly SpanLimits $spanLimits, private readonly SamplerInterface $sampler, array $spanProcessors, ?MeterProviderInterface $meterProvider = null)
    {
        ...
        if ($meterProvider !== null) {
            ...
            $this->spanStartedCounter = $meter->createCounter(OtelIncubatingMetrics::OTEL_SDK_SPAN_STARTED, '{span}', 'The number of created spans');
            $this->spanLiveCounter = $meter->createUpDownCounter(OtelIncubatingMetrics::OTEL_SDK_SPAN_LIVE, '{span}', 'The number of created spans with recording=true for which the end operation has not been called yet');
        } else {
