...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Trace\SpanSuppression\NoopSuppressionStrategy\NoopSuppressor;
...
final class SpanBuilder implements API\SpanBuilderInterface
{
    ...
    public function __construct(private readonly string $spanName, private readonly InstrumentationScopeInterface $instrumentationScope, private readonly TracerSharedState $tracerSharedState, private readonly SpanSuppressor $spanSuppressor = new NoopSuppressor())
    {
    ...
    public function startSpan(): API\SpanInterface
    {
        ...
        $this->tracerSharedState->getSpanStartedCounter()?->add(1, [OtelIncubatingAttributes::OTEL_SPAN_SAMPLING_RESULT => $samplingResultAttr, OtelIncubatingAttributes::OTEL_SPAN_PARENT_ORIGIN => $parentOriginAttr]);
        if (!in_array($samplingDecision, [SamplingResult::RECORD_AND_SAMPLE, SamplingResult::RECORD_ONLY], \true)) {
