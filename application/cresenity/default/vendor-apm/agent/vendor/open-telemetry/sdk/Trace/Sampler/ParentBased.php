...
class ParentBased implements SamplerInterface
{
     ...
     */
    public function __construct(private readonly SamplerInterface $root, ?SamplerInterface $remoteParentSampler = null, ?SamplerInterface $remoteParentNotSampler = null, ?SamplerInterface $localParentSampler = null, ?SamplerInterface $localParentNotSampler = null)
    {
    ...
    public function shouldSample(ContextInterface $parentContext, string $traceId, string $spanName, int $spanKind, AttributesInterface $attributes, array $links): SamplingResult
    {
        ...
        if ($parentSpanContext->isRemote()) {
            return $parentSpanContext->isSampled() ? $this->remoteParentSampler->shouldSample(...func_get_args()) : $this->remoteParentNotSampler->shouldSample(...func_get_args());
        }
        return $parentSpanContext->isSampled() ? $this->localParentSampler->shouldSample(...func_get_args()) : $this->localParentNotSampler->shouldSample(...func_get_args());
    }
    #[\Override]
