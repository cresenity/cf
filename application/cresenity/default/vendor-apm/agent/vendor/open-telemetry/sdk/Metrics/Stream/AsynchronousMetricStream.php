...
final class AsynchronousMetricStream implements MetricStreamInterface
{
    ...
    public function __construct(private readonly AggregationInterface $aggregation, private readonly int $startTimestamp)
    {
    ...
    public function register($temporality): int
    {
        ...
        if (($reader = array_search(null, $this->lastReads, \true)) === \false) {
    ...
    public function collect(int $reader): DataInterface
    {
        ...
        return $this->aggregation->toData($metric->attributes, $metric->summaries, Exemplar::groupByIndex($metric->exemplars), $startTimestamp, $metric->timestamp, $temporality);
    }
