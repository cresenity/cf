...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Logs\ReadableLogRecord;
...
class ConsoleExporter implements LogRecordExporterInterface
{
    ...
    public function export(iterable $batch, ?CancellationInterface $cancellation = null): FutureInterface
    {
        ...
        $output = ['resource' => $resource, 'scopes' => array_values($scopes)];
        $payload = json_encode($output, \JSON_PRETTY_PRINT);
        if ($payload === \false) {
            return new ErrorFuture(new \RuntimeException('Failed to encode log records to JSON: ' . json_last_error_msg()));
        }
    ...
    {
        return ['name' => $scope->getName(), 'version' => $scope->getVersion(), 'attributes' => $scope->getAttributes()->toArray(), 'dropped_attributes_count' => $scope->getAttributes()->getDroppedAttributesCount(), 'schema_url' => $scope->getSchemaUrl(), 'logs' => []];
    }
}
