...
use CresenityDevCloudAPMVendor\OpenTelemetry\API\Signals;
...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Resource\Detectors\Sdk;
use CresenityDevCloudAPMVendor\OpenTelemetry\SemConv\ResourceAttributes;
...
class OtlpUtil
{
     ...
     */
    private const METHODS = [Signals::TRACE => '/opentelemetry.proto.collector.trace.v1.TraceService/Export', Signals::METRICS => '/opentelemetry.proto.collector.metrics.v1.MetricsService/Export', Signals::LOGS => '/opentelemetry.proto.collector.logs.v1.LogsService/Export'];
    private const HEADER_VARS = [Signals::TRACE => Variables::OTEL_EXPORTER_OTLP_TRACES_HEADERS, Signals::METRICS => Variables::OTEL_EXPORTER_OTLP_METRICS_HEADERS, Signals::LOGS => Variables::OTEL_EXPORTER_OTLP_LOGS_HEADERS];
    ...
    public static function getUserAgentHeader(): array
    {
        ...
        if ($header === null) {
            ...
            $header = ['User-Agent' => sprintf('OTel OTLP Exporter PHP/%s', $resource->getAttributes()->get(ResourceAttributes::TELEMETRY_SDK_VERSION) ?: 'unknown')];
        }
        return $header;
    }
}
