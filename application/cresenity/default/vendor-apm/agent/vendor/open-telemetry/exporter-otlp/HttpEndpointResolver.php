...
use CresenityDevCloudAPMVendor\OpenTelemetry\SDK\Common\Adapter\HttpDiscovery\MessageFactoryResolver;
...
class HttpEndpointResolver implements HttpEndpointResolverInterface
{
    ...
    {
        return $this->httpFactoryResolver->resolveUriFactory()->createUri();
    }
    private function createDefaultUri(array $components, string $signal): UriInterface
    {
        ...
        return $this->createUri()->withScheme($components[self::SCHEME_ATTRIBUTE] ?? self::DEFAULT_SCHEME)->withPath(self::resolvePath($components[self::PATH_ATTRIBUTE] ?? self::ROOT_PATH, $signal))->withHost($components[self::HOST_ATTRIBUTE]);
    }
