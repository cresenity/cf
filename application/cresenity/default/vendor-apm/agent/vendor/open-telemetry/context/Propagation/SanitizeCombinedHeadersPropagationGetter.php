...
final class SanitizeCombinedHeadersPropagationGetter implements ExtendedPropagationGetterInterface
{
    ...
    public function get($carrier, string $key): ?string
    {
        ...
        return preg_replace([self::SERVER_CONCAT_HEADERS_REGEX, self::TRAILING_LEADING_SEPARATOR_REGEX], [self::LIST_MEMBERS_SEPARATOR], $value);
    }
    /**
    ...
    {
        $value = $this->getter instanceof ExtendedPropagationGetterInterface ? $this->getter->getAll($carrier, $key) : (array) $this->getter->get($carrier, $key);
        if ($value === []) {
            return [];
        }
        $value = preg_replace([self::SERVER_CONCAT_HEADERS_REGEX, self::TRAILING_LEADING_SEPARATOR_REGEX], [self::LIST_MEMBERS_SEPARATOR], $value);
