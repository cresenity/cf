...
class CServer_Runner_Command {
    ...
    /**
     * @var null|string the initial working dir for `proc_open()`. Default is
     *                  `null` for current PHP working dir.
     */
    public $procCwd;

    /**
     * @var null|array an array with environment variables to pass to
     *                 `proc_open()`. Default is `null` for none.
     */
    public $procEnv;

    /**
     * @var null|array an array of other_options for `proc_open()`. Default is
     *                 `null` for none.
     */
    public $procOptions;

    /**
     * @var null|bool whether to set the stdin/stdout/stderr streams to
    ...
    /**
     * @return null|string the command that was set through setCommand() or
     ...
     *                                  and $escape is `true`.
     * @param null|string|array $value  the optional argument value which will
     ...
     *                                  'val2'`.
     * @param null|bool         $escape if set, this overrides the $escapeArgs setting
    ...
    /**
     * @return null|int the exit code or null if command was not executed yet
     */
    ...
    public function execute() {
        ...
        if ($this->useSymfony) {
            ...
            if ($this->_exitCode !== 0) {
                $this->_stdErr = $process->getErrorOutput();
                $this->_error = $process->getErrorOutput();
