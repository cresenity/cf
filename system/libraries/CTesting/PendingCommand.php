...
class CTesting_PendingCommand {
     ...
     * @return void
     */
    public function __construct(PHPUnitTestCase $test, $command, $parameters) {
        $this->test = $test;
    ...
    public function assertSuccessful() {
        return $this->assertExitCode(CConsole_Command::SUCCESS);
    }
    ...
    public function assertFailed() {
        return $this->assertNotExitCode(CConsole_Command::SUCCESS);
    }
    ...
    public function run() {
        ...
        try {
            ...
            $exitCode = $kernel->call($this->command, $this->parameters, $mock);
    ...
    protected function verifyExpectations() {
        if (count($this->test->expectedQuestions)) {
            $this->test->fail('Question "' . carr::first($this->test->expectedQuestions)[0] . '" was not asked.');
        }
        ...
        if (count($this->test->expectedOutput)) {
            $this->test->fail('Output "' . carr::first($this->test->expectedOutput) . '" was not printed.');
        }

        if (count($this->test->expectedOutputSubstrings)) {
            $this->test->fail('Output does not contain "' . carr::first($this->test->expectedOutputSubstrings) . '".');
        }
