...
class CCache_Driver_DynamoDbDriver extends CCache_DriverAbstract implements CCache_Contract_LockProviderDriverInterface {
    ...
    public function get($key) {
        ...
        if (isset($response['Item'][$this->valueAttribute])) {
            return $this->unserialize(
                isset($response['Item'][$this->valueAttribute]['S'])
                ? $response['Item'][$this->valueAttribute]['S']
                : (isset($response['Item'][$this->valueAttribute]['N']) ? $response['Item'][$this->valueAttribute]['N'] : null)
            );
        }
    }
    ...
    public function many(array $keys) {
        ...
        $response = $this->dynamo->batchGetItem([
            'RequestItems' => [
                $this->table => [
                    'ConsistentRead' => false,
                    'Keys' => c::collect($prefixedKeys)->map(function ($key) {
                        return [
                            $this->keyAttribute => [
                                'S' => $key,
                            ],
        ...
        return array_merge(c::collect(array_flip($keys))->map(function () {
        })->all(), c::collect($response['Responses'][$this->table])->mapWithKeys(function ($response) use ($now) {
            if ($this->isExpired($response, $now)) {
                $value = null;
            } else {
                $value = $this->unserialize(
                    isset($response[$this->valueAttribute]['S'])
                    ? $response[$this->valueAttribute]['S']
                    : (isset($response[$this->valueAttribute]['N']) ? $response[$this->valueAttribute]['N'] : null)
                );
            }

            return [cstr::replaceFirst($this->prefix, '', $response[$this->keyAttribute]['S']) => $value];
        })->all());
    }
    ...
    protected function isExpired(array $item, $expiration = null) {
        $expiration = $expiration ?: CCarbon::now();
    ...
    public function putMany(array $values, $seconds) {
        ...
        $this->dynamo->batchWriteItem([
            'RequestItems' => [
                $this->table => c::collect($values)->map(function ($value, $key) use ($expiration) {
                    return [
                        'PutRequest' => [
                            'Item' => [
                                $this->keyAttribute => [
        ...
        try {
            $this->dynamo->putItem([
                ...
                'ExpressionAttributeValues' => [
                    ':now' => [
                        'N' => (string) CCarbon::now()->getTimestamp(),
                    ],
                ],
            ]);
            ...
            if (cstr::contains($e->getMessage(), 'ConditionalCheckFailed')) {
                return false;
            }
    ...
    public function forever($key, $value) {
        return $this->put($key, $value, CCarbon::now()->addYears(5)->getTimestamp());
    }
    ...
    public function lock($name, $seconds = 0, $owner = null) {
        return new CCache_Lock_DynamoDbLock($this, $this->prefix . $name, $seconds, $owner);
    }
