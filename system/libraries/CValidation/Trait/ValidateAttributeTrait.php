...
trait CValidation_Trait_ValidateAttributeTrait {
    ...
    protected function getDateTime($value) {
        try {
            return @CCarbon::parse($value) ?: null;
        } catch (Exception $e) {
        }
    }
    ...
    protected function validateCurrentPassword($attribute, $value, $parameters) {
        ...
        $guard = $auth->guard(carr::first($parameters));
    ...
    public function validateDifferent($attribute, $value, $parameters) {
        ...
        foreach ($parameters as $parameter) {
            if (carr::has($this->data, $parameter)) {
    ...
    public function validateDistinct($attribute, $value, $parameters) {
        $data = carr::except($this->getDistinctValues($attribute), $attribute);
    ...
    protected function extractDistinctValues($attribute) {
        $attributeData = CValidation_Data::extractDataFromPath(
            CValidation_Data::getLeadingExplicitAttributePath($attribute),
            $this->data
        );

        $pattern = str_replace('\*', '[^.]+', preg_quote($attribute, '#'));

        return carr::where(carr::dot($attributeData), function ($value, $key) use ($pattern) {
    ...
    public function validateEmail($attribute, $value, $parameters) {
        ...
        $validations = c::collect($parameters)
            ->unique()
            ->map(function ($validation) {
                ...
                if ($validation == 'filter') {
                    return new CValidation_FilterEmailValidation();
                }
    ...
    protected function getExistCount($connection, $table, $column, $value, $parameters) {
        ...
        if ($this->currentRule instanceof CValidation_Rule_Exists) {
    ...
    public function validateUnique($attribute, $value, $parameters) {
        ...
        list($connection, $table, $idColumn) = $this->parseTable($parameters[0]);
    ...
    protected function parseTable($table) {
        list($connection, $table) = cstr::contains($table, '.') ? explode('.', $table, 2) : [null, $table];

        if (str_contains($table, '\\') && class_exists($table) && is_a($table, CModel::class, true)) {
            $model = new $table();

            $table = $model->getTable();
            $connection ??= $model->getConnectionName();

            if (str_contains($table, '.') && cstr::startsWith($table, $connection)) {
                $connection = null;
            }
    ...
    public function guessColumnForQuery($attribute) {
        if (in_array($attribute, carr::collapse($this->implicitAttributes))
            && !is_numeric($last = carr::last(explode('.', $attribute)))
        ) {
            return $last;
        }
    ...
    public function validateMultipleOf($attribute, $value, $parameters) {
        ...
        try {
            ...
            throw new CBase_Exception_MathException('An error occurred while handling the multiple_of input values.', 0, $e);
        }
    }
    ...
    public function validateRequiredIf($attribute, $value, $parameters) {
        ...
        if (!carr::has($this->data, $parameters[0])) {
            return true;
        }
    ...
    public function validateProhibits($attribute, $value, $parameters) {
        if ($this->validateRequired($attribute, $value)) {
            foreach ($parameters as $parameter) {
                if ($this->validateRequired($parameter, carr::get($this->data, $parameter))) {
                    return false;
                }
            }
        }
    ...
    protected function shouldConvertToBoolean($parameter) {
        return in_array('boolean', carr::get($this->rules, $parameter, []));
    }
    ...
    protected function convertValuesToNull($values) {
        return array_map(function ($value) {
            return cstr::lower($value) === 'null' ? null : $value;
        }, $values);
    }
