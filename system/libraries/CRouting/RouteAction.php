...
class CRouting_RouteAction {
    ...
    public static function parse($uri, $action) {
        ...
        if (CBase_Reflector::isCallable($action, true)) {
    ...
    protected static function findCallable(array $action) {
        return carr::first($action, function ($value, $key) {
                    return CBase_Reflector::isCallable($value) && is_numeric($key);
                });
    }
    ...
    public static function containsSerializedClosure(array $action) {
        return is_string($action['uses']) &&
                cstr::startsWith($action['uses'], 'C:32:"Opis\\Closure\\SerializableClosure') !== false;
    }
