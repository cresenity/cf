...
class CReport_Jasper_Element_Rectangle extends CReport_Jasper_Element {
    public function generate(CReport_Jasper_Report $report) {
        ...
        if (isset($data->graphicElement->pen['lineColor'])) {
            $drawcolor = ['r' => hexdec(substr($data->graphicElement->pen['lineColor'], 1, 2)), 'g' => hexdec(substr($data->graphicElement->pen['lineColor'], 3, 2)), 'b' => hexdec(substr($data->graphicElement->pen['lineColor'], 5, 2))];
        }
        ...
        $border = ['width' => $borderwidth, 'color' => $drawcolor, 'cap' => 'square',
        ...
        if (isset($data->reportElement['backcolor']) && ($mode == 'Opaque' || $mode == '')) {
            $fillcolor = ['r' => hexdec(substr($data->reportElement['backcolor'], 1, 2)), 'g' => hexdec(substr($data->reportElement['backcolor'], 3, 2)), 'b' => hexdec(substr($data->reportElement['backcolor'], 5, 2))];
        } else {
        ...
        // else
        $report->getInstructions()->addInstruction(CReport_Jasper_Instruction::TYPE_ROUNDED_RECT, [
