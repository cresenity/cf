...
final class CGeo_Provider_GoogleMaps extends CGeo_ProviderHttpAbstract implements CGeo_Interface_ProviderInterface {
    ...
    public function geocodeQuery(CGeo_Query_GeocodeQuery $query) {
        ...
        if (filter_var($query->getText(), FILTER_VALIDATE_IP)) {
            throw new CGeo_Exception_UnsupportedOperation('The GoogleMaps provider does not support IP addresses, only street addresses.');
        }
     ...
     * @throws CGeo_Exception_InvalidCredentials
     *
     ...
     *
     * @param CGeo_Model_AddressBuilder $builder
     * @param string                    $type    Component type
     * @param object                    $values  The component values
     */
    private function updateAddressComponent(CGeo_Model_AddressBuilder $builder, string $type, $values) {
        switch ($type) {
            case 'postal_code':
     ...
     * @throws CGeo_Exception_InvalidCredentials
     * @throws CGeo_Exception_InvalidServerResponse
     * @throws CGeo_Exception_QuotaExceeded
     *
    ...
    private function validateResponse(string $url, $content) {
        ...
        if (false !== strpos($content, "Provided 'signature' is not valid for the provided client ID")) {
            throw new CGeo_Exception_InvalidCredentials(sprintf('Invalid client ID / API Key %s', $url));
        }
        ...
        if ('REQUEST_DENIED' === $json->status && 'The provided API key is invalid.' === $json->error_message) {
            throw new CGeo_Exception_InvalidCredentials(sprintf('API key is invalid %s', $url));
        }

        if ('REQUEST_DENIED' === $json->status) {
            throw new CGeo_Exception_InvalidServerResponse(sprintf('API access denied. Request: %s - Message: %s', $url, $json->error_message));
        }

        // you are over your quota
        if ('OVER_QUERY_LIMIT' === $json->status) {
            throw new CGeo_Exception_QuotaExceeded(sprintf('Daily quota exceeded %s', $url));
        }
     ...
     */
    private function parseCoordinates(CGeo_Model_AddressBuilder $builder, $result) {
