...
class CLogger_Reader_Viewer {
    ...
    protected function getFilePaths(): array {
        ...
        foreach (CF::config('log.reader.include_files', []) as $pattern) {
            if (!cstr::startsWith($pattern, DIRECTORY_SEPARATOR)) {
        ...
        foreach (CF::config('log.reader.exclude_files', []) as $pattern) {
            if (!cstr::startsWith($pattern, DIRECTORY_SEPARATOR)) {
                $pattern = $baseDir . $pattern;
            }
    ...
    public function basePathForLogs(): string {
        return cstr::finish(realpath(storage_path('logs')), DIRECTORY_SEPARATOR);
    }
