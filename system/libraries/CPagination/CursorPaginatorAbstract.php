 ...
 * @mixin \CCollection
 */
abstract class CPagination_CursorPaginatorAbstract implements CInterface_Htmlable {
    use CTrait_ForwardsCalls, CTrait_Tappable;
     ...
     * @param bool                   $isNext
     *
     * @return \CPagination_Cursor
     */
    public function getCursorForItem($item, $isNext = true) {
        return new CPagination_Cursor($this->getParametersForItem($item), $isNext);
    }
     ...
     *
     * @param \ArrayAccess|\stdClass|\CModel $item
     * @param string                         $parameterName
     *
     * @return null|string
     */
    protected function getPivotParameterForItem($item, $parameterName) {
        $table = cstr::beforeLast($parameterName, '.');

        foreach ($item->getRelations() as $relation) {
            if ($relation instanceof CModel_Relation_Pivot && $relation->getTable() === $table) {
                return $this->ensureParameterIsPrimitive(
                    $relation->getAttribute(cstr::afterLast($parameterName, '.'))
                );
            }
        }
    }
    ...
    public static function viewFactory() {
        return CPagination_Paginator::viewFactory();
    }
