 ...
 * @version Release:1.0
 */
class CXMPP_XMPP extends CXMPP_XMLStream {
    /**
     * @var string
     */
    public $server;
    ...
    public function message_handler($xml) {
        ...
        $payload['xml'] = $xml;
        $this->log->log("Message: {$xml->sub('body')->data}", CXMPP_Log::LEVEL_DEBUG);
    ...
    public function presence_handler($xml) {
        ...
        $this->log->log("Presence: {$payload['from']} [{$payload['show']}] {$payload['status']}", CXMPP_Log::LEVEL_DEBUG);
    ...
    protected function sasl_failure_handler($xml) {
        $this->log->log('Auth failed!', CXMPP_Log::LEVEL_ERROR);
        $this->disconnect();

        throw new CXMPP_Exception('Auth failed!');
    }
