...
class CHTTP_Client_ResponseSequence {
    ...
    public function pushStatus($status, array $headers = []) {
        return $this->pushResponse(
            CHTTP_Client::response('', $status, $headers)
        );
    }
    ...
    public function pushFile($filePath, $status = 200, array $headers = []) {
        ...
        return $this->pushResponse(
            CHTTP_Client::response($string, $status, $headers)
        );
    }
    ...
    public function dontFailWhenEmpty() {
        return $this->whenEmpty(CHTTP_Client::response());
    }
    ...
    public function __invoke() {
        ...
        if (!$this->failWhenEmpty && count($this->responses) === 0) {
            return c::value($this->emptyResponse ?? CHTTP_Client::response());
        }
