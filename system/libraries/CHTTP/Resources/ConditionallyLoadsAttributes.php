...
trait CHTTP_Resources_ConditionallyLoadsAttributes {
    /**
    ...
    protected function filter($data) {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_numeric($key) && $value instanceof CHTTP_Resources_MergeValue) {
    ...
    protected function removeMissingValues($data) {
        ...
        foreach ($data as $key => $value) {
            if (($value instanceof CHTTP_Resources_PotentiallyMissing && $value->isMissing())
                || ($value instanceof self
                && $value->resource instanceof CHTTP_Resources_PotentiallyMissing
                && $value->isMissing())
            ) {
                unset($data[$key]);
            } else {
     ...
     * @param mixed $default
     *
     * @return \CHTTP_Resources_MissingValue|mixed
     */
    protected function when($condition, $value, $default = null) {
        ...
        return func_num_args() === 3 ? c::value($default) : new CHTTP_Resources_MissingValue();
    }
     ...
     * @param mixed $value
     *
     * @return \CHTTP_Resources_MergeValue|mixed
     */
    ...
    protected function mergeWhen($condition, $value, $default = null) {
        if ($condition) {
            return new CHTTP_Resources_MergeValue(c::value($value));
        }

        return func_num_args() === 3 ? new CHTTP_Resources_MergeValue(c::value($default)) : new CHTTP_Resources_MissingValue();
    }
     ...
     *
     * @return \CHTTP_Resources_MergeValue
     */
    protected function attributes($attributes) {
        return new CHTTP_Resources_MergeValue(
    ...
    public function whenHas($attribute, $value = null, $default = null) {
        ...
        return func_num_args() === 1
            ...
            : c::value($value, $this->resource->{$attribute});
    }
    ...
    protected function whenAppended($attribute, $value = null, $default = null) {
        if ($this->resource->hasAppended($attribute)) {
            return func_num_args() >= 2 ? c::value($value) : $this->resource->$attribute;
        }
    ...
    protected function whenLoaded($relationship, $value = null, $default = null) {
        ...
        if ($value === null) {
            $value = c::value($relationship, $value, $default);
        }

        return c::value($value, $loadedValue);
    }
    ...
    public function whenCounted($relationship, $value = null, $default = null) {
        ...
        return c::value($value, $this->resource->{$attribute});
    }
    ...
    public function whenAggregated($relationship, $column, $aggregate, $value = null, $default = null) {
        ...
        if ($value === null) {
            $value = c::value($relationship, $column, $aggregate, $value, $default);
        }
    ...
    protected function transform($value, callable $callback, $default = null) {
        return c::transform($value, $callback, func_num_args() === 3 ? $default : new CHTTP_Resources_MissingValue());
    }
}
