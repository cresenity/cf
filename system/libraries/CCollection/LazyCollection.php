...
class CCollection_LazyCollection implements CInterface_Enumerable, CBase_Contract_CanBeEscapedWhenCastToStringInterface {
    ...
    public function collapse() {
        return new static(function () {
            foreach ($this as $values) {
                if (is_array($values) || $values instanceof CInterface_Enumerable) {
    ...
    public function flatten($depth = INF) {
        $instance = new static(function () use ($depth) {
            foreach ($this as $item) {
                if (!is_array($item) && !$item instanceof CInterface_Enumerable) {
                    yield $item;
                } elseif ($depth === 1) {
                    yield from $item;
                } else {
    ...
    public function last(?callable $callback = null, $default = null) {
        ...
        return $needle === $placeholder ? c::value($default) : $needle;
    }
    ...
    public function zip($items) {
        ...
        return new static(function () use ($iterables) {
            $iterators = CCollection::make($iterables)->map(function ($iterable) {
