...
class CCollection implements ArrayAccess, CInterface_Enumerable, CBase_Contract_CanBeEscapedWhenCastToStringInterface {
    ...
    public function lazy() {
        return new CCollection_LazyCollection($this->items);
    }
    ...
    public function contains($key, $operator = null, $value = null) {
        if (func_num_args() === 1 || (is_callable($key)) || ($operator === null && $value === null)) {
     ...
     *
     * @param \CCollection|mixed $keys
     *
     * @return static
     */
    public function except($keys) {
        if ($keys instanceof CInterface_Enumerable) {
            $keys = $keys->all();
     ...
     *
     * @param null|string|array|int $value
    ...
    public function prepend($value, $key = null) {
        $this->items = carr::prepend($this->items, ...func_get_args());
    ...
    public function shuffle($seed = null) {
        return new static(carr::shuffle($this->items, $seed));
    }
    ...
    public function sole($key = null, $operator = null, $value = null) {
        ...
        if ($items->count() > 1) {
            throw new CCollection_Exception_MultipleItemsFoundException();
        }
    ...
    protected function sortByMany(array $comparisons = []) {
        ...
        usort($items, function ($a, $b) use ($comparisons) {
            foreach ($comparisons as $comparison) {
                $comparison = carr::wrap($comparison);

                $prop = $comparison[0];

                $ascending = carr::get($comparison, 1, true) === true
                             || carr::get($comparison, 1, true) === 'asc';
                ...
                if (is_callable($prop)) {
                    ...
                    $values = [c::get($a, $prop), c::get($b, $prop)];

                    if (!$ascending) {
                        $values = array_reverse($values);
                    }

                    $result = c::spaceshipOperator($values[0], $values[1]);
                }
    ...
    public function undot() {
        return new static(carr::undot($this->all()));
    }
